...
class JellyfinInstanceModal extends Modal {
  ...
  onOpen() {
    ...
    new Setting(contentEl)
      .setName("API key")
      .setDesc("Jellyfin API key (generate in Dashboard G�� API keys)")
      .addText((text) => {
        text
...
export class WhoIsStreamingSettingsTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    let buttonText = "";
    if (this.plugin.settings.plex?.accessToken) {
      plexDescEl.appendText("Edit your Plex integration");
      buttonText = "Edit";
    } else {
      plexDescEl.appendText("Add Plex account to check each server for movie availability");
      buttonText = "Add";
    }
    // const plexDescEl = plexDescription.createDiv({ cls: "setting-item-description" });
    // plexDescEl.appendText("Add Plex account to check each server for movie availability");

    var plexSettings = new Setting(containerEl)
    ...
    if (this.plugin.settings.plex.accessToken) {
      plexSettings.addButton((button) => {
        button
          .setButtonText("Disconnect")
          .setWarning()
          .onClick(() => {
            void (async () => {
              this.plugin.settings.plex.accessToken = "";
              // this.plugin.settings.plex.appName = "";
              // this.plugin.settings.plex.clientIdentifier = "";
              await this.plugin.saveSettings();
              this.display();
            })();
          });
